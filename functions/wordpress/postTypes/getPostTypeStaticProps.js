import { handleError } from '@/functions/utility'
import getFrontendPage from '@/functions/wordpress/postTypes/getFrontendPage'
import getGlobalPageConfig from '@/functions/wordpress/postTypes/getGlobalPageConfig'
...
import getPostTypeProduct from '@/functions/wordpress/postTypes/getPostTypeProduct'
import getPostTypeSupportProduct from '@/functions/wordpress/postTypes/getPostTypeSupportProduct'
...
export default async function getPostTypeStaticProps(
  ...
  const revalidate = Number(process.env.NEXT_STATIC_PROPS_REVALIDATE)
  ...
  if (Object.keys(frontendPageSeo).includes(postType)) {
    const { apolloClient, ...routeData } = await getFrontendPage(
      postType,
      language
    )
  ...
  if (!Object.keys(params).length) {
    const { apolloClient, ...archiveData } = await getPostTypeArchive(
      postType,
      'DATE',
      'DESC',
      null,
  ...
  /* -- Handle pages set via Additional Settings. -- */
  switch (postType) {
    case 'globalpageconfig': {
      const { apolloClient, ...pageData } = await getGlobalPageConfig(
        slug,
        language
      )
      return addApolloState(apolloClient, {
    ...
    case 'supportproduct': {
      const { apolloClient, error, ...pageData } =
        await getPostTypeSupportProduct(slug, {
          ...params,
          language,
    ...
    case 'product': {
      const { apolloClient, ...pageData } = await getPostTypeProduct(slug, {
        ...params,
        language,
        menuLanguage: language
      })

      // Display relevant error page if error encountered.
      if (pageData?.error) {
        return handleError(pageData?.errorCode, pageData?.errorMessage)
      }
  ...
  // Display relevant error page if error encountered.
  if (props?.error) {
    return handleError(postData?.errorCode, postData?.errorMessage)
  }

...
import ProductDetailSkeleton from '@/components/molecules/Product/Skeleton/ProductDetailSkeleton'
...
import {formatProductAddons} from '@/functions/product/productUtil'
...
export default function ProductPost({
  ...
  useEffect(() => {
    ...
    TagManager.dataLayer({
      dataLayer: prepareTagManagerDataLayerObj(post, 'view_item')
    })
  }, [])

  return (
    <LayoutNew seo={{...seo}} hasJsonLd={true}>
      <Container>
        {!router.isFallback ? (
          <ProductDetails
            product={post}
            productAddons={formatProductAddons(productAddons)}
            support={supportContactSection}
            postType={postType}
            slug={slug}
            queryParams={rest}
          />
